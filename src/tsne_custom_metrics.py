...
def convex_hull_intersection_area(hull1, hull2):
    intersect_poly = sutherland_hodgman(hull1, hull2)
...
class CustomMetrics:
    ...
    def calculate_cluster_score(self, hulls=None):
        ...
        total_area = 0
        for i in hulls:
            total_area += polygon_area(hulls[i].hull)

        hull_points = [hulls[i].hull for i in hulls]
        intersection_area = 0
        for i in range(len(hull_points)):
            for j in range(i + 1, len(hull_points)):
                intersection_area += convex_hull_intersection_area(hull_points[i], hull_points[j])
